...
ためのワークフローをまとめたものです。  
初期の Issue 作成〜ファーストコミット〜Draft PR の材料整理までは `stories-three-subjects-workflow` と `blogs-issue-pr-workflow` のルールに従い、その続きとして使います。

このスキルは、完成した作品に合わせて Issue / PR に何を書くかを決める。GitHub 上での更新操作そのものは別の汎用手順で行う。
...
## Step 5: クローズに向けて必要情報を整理する

1. Issue と PR の紐付けに必要な情報が揃っているか確認する。
   - タイトルの `#<id>` だけでなく、必要に応じて PR 本文に `Closes #<id>` を追加してもよい。
2. マージ前に残したい情報があれば、Issue または PR に追記する材料として整理する。
   - 例: 公開 URL、作品タイトル、三題、掲載先
